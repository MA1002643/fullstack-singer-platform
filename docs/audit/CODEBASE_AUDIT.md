# Codebase Audit — Imran Khan Singer Website
...
|---|---|---|---|
| `index.html` | `/` | Home — navbar + 4-image photo card gallery | `<title>` is "Website Navbar" |
| `Singer Website/HTML/about.html` | about | Artist biography (career, "Ni Nachleh", "Amplifier", "Bewafa") | Substantive, accurate copy worth keeping |
| `Singer Website/HTML/songs.html` | songs | 4 YouTube embeds (`uuCFRaFWjwY`, `JYodEWUdIso`, `iP872SycxjI`, `qRTG8uF2ES4`) | Playback happens on YouTube's licensed infra |
| `Singer Website/HTML/sign-up.html` | sign-up | Mailing-list form (name + email) | Posts to third-party university server |
...
`Singer Website/JavaScript/app.js` (~90 lines):
...
`Singer Website/Public/style.css` (~545 lines), one global sheet, no methodology (part-BEM, part ad-hoc):
...
| `"Singer Website"` path scheme (spaces, absolute `/Singer Website/...` URLs) | Breaks under any sub-path hosting; hostile to tooling |
...
4. Correct page `<title>`s, add meta descriptions and a favicon.
